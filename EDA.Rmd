...
Se observa que no existen valores faltantes en la base de datos. Se procedió a la revisión de datos atípicos.
...
La temperatura tiene zeros y negativos, pero es lógico por algunos países como EU, Canadá o Europa que tienen climas extremadamente frios., lo cual no representa tema. de la misma forma los demás valores se notan correctos al menos a nivel macro.

```{r}
df_long
```

### Agrupación de df_long a nivel mensual por Continente, País y por Ciudad

Se agrupo la información en tres conjuntos
 - Mensual Continental
 - Mensual por País
 - Mensual por Ciudad.
...
# Transformación de la columna Fecha en formato tipo Date
df_long <- df_long %>%
  mutate(Date = as.Date(Date)) 

df_mensual_Country <- df_long %>%
...
df_mensual_City <- df_long %>%
  mutate(YearMonth = format(Date, "%Y-%m")) %>%  # formato Año-Mes como "2023-12"
  group_by(YearMonth, City, Country, Continent, Metrica) %>%
  summarise(
    promedio = mean(valor, na.rm = TRUE),
    maximo = max(valor, na.rm = TRUE),
    minimo = min(valor, na.rm = TRUE),
...
df_mensual_continental <- df_long %>%
  mutate(YearMonth = format(Date, "%Y-%m")) %>%  # formato Año-Mes como "2023-12"
  group_by(YearMonth, Continent, Metrica) %>%
  summarise(
    promedio = mean(valor, na.rm = TRUE),
    maximo = max(valor, na.rm = TRUE),
    minimo = min(valor, na.rm = TRUE),
...
continentes <- unique(df_mensual_continental$Continent)

# Bucle por continente
for (cont in continentes) {
  df_filtrado <- df_mensual_continental %>% filter(Continent == cont) 
  
  p <- ggplot(df_filtrado, aes(x = YearMonth, y = promedio, color = Metrica, group = Metrica)) +
    geom_line() +
    scale_color_manual(values = colores_metricas) + 
...
Se observa que los continentes como South América y Oceanía son los peores por tener altos índices de 
...
Se observa que la información es "consistente" al menos a nivel Continente, seguiré explorando la información por Ciudad
